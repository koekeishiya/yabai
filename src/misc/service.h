...
#define _nimbuswm_PLIST                                                                                            \
    "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"                                                                 \
    "<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">\n" \
    "<plist version=\"1.0\">\n"                                                                                    \
    "<dict>\n"                                                                                                     \
    "    <key>Label</key>\n"                                                                                       \
    "    <string>"_NAME_nimbuswm_PLIST                                                                             \
    "</string>\n"                                                                                                  \
    ...
    "    <string>/tmp/nimbuswm_%s.out.log</string>\n"                                                              \
    "    <key>StandardErrorPath</key>\n"                                                                           \
    "    <string>/tmp/nimbuswm_%s.err.log</string>\n"                                                              \
    "    <key>ProcessType</key>\n"                                                                                 \
...
static char *populate_plist_path(void)
{
    ...
    if (!home)
    {
        error("nimbuswm: unable to retrieve home directory! abort..\n");
    }

    int size = strlen(_PATH_nimbuswm_PLIST) - 2 + strlen(home) + 1;
    char *result = malloc(size);
    if (!result)
    {
        error("nimbuswm: could not allocate memory for plist path! abort..\n");
    }

    memset(result, 0, size);
    snprintf(result, size, _PATH_nimbuswm_PLIST, home);
...
static char *populate_plist(int *length)
{
    ...
    if (!path_env)
    {
        error("nimbuswm: 'env PATH' not set! abort..\n");
    }
    ...
    {
        error("nimbuswm: unable to retrieve path of executable! abort..\n");
    }

    int size = strlen(_nimbuswm_PLIST) - 8 + strlen(exe_path) + strlen(path_env) + (2 * strlen(user)) + 1;
    char *result = malloc(size);
    if (!result)
    {
        error("nimbuswm: could not allocate memory for plist contents! abort..\n");
    }

    memset(result, 0, size);
    snprintf(result, size, _nimbuswm_PLIST, exe_path, path_env, user, user);
    *length = size - 1;
...
static inline void ensure_directory_exists(char *nimbuswm_plist_path)
{
    ...
    char *last_slash = strrchr(nimbuswm_plist_path, '/');
    *last_slash = '\0';

    if (!directory_exists(nimbuswm_plist_path))
    {
        mkdir(nimbuswm_plist_path, 0755);
    }
...
static int service_install_internal(char *nimbuswm_plist_path)
{
    int nimbuswm_plist_length;
    char *nimbuswm_plist = populate_plist(&nimbuswm_plist_length);
    ensure_directory_exists(nimbuswm_plist_path);

    FILE *handle = fopen(nimbuswm_plist_path, "w");
    if (!handle)
        return 1;

    size_t bytes = fwrite(nimbuswm_plist, nimbuswm_plist_length, 1, handle);
...
{
    char *nimbuswm_plist_path = populate_plist_path();

    if (file_exists(nimbuswm_plist_path))
    {
        error("nimbuswm: service file '%s' is already installed! abort..\n", nimbuswm_plist_path);
    }

    return service_install_internal(nimbuswm_plist_path);
}
...
static int service_uninstall(void)
{
    ...
    if (!file_exists(nimbuswm_plist_path))
    {
        error("nimbuswm: service file '%s' is not installed! abort..\n", nimbuswm_plist_path);
    }

    return unlink(nimbuswm_plist_path) == 0 ? 0 : 1;
}
...
static int service_start(void)
{
    ...
    if (!file_exists(nimbuswm_plist_path))
    {
        warn("nimbuswm: service file '%s' is not installed! attempting installation..\n", nimbuswm_plist_path);

        int result = service_install_internal(nimbuswm_plist_path);
        if (result)
        {
            error("nimbuswm: service file '%s' could not be installed! abort..\n", nimbuswm_plist_path);
        }
    }

    char service_target[MAXLEN];
    snprintf(service_target, sizeof(service_target), "gui/%d/%s", getuid(), _NAME_nimbuswm_PLIST);
    ...
    if (is_bootstrapped != 0)
    {
        ...
        const char *const args2[] = {_PATH_LAUNCHCTL, "bootstrap", domain_target, nimbuswm_plist_path, NULL};
...
static int service_stop(void)
{
    ...
    else
    {
        ...
        const char *const args[] = {_PATH_LAUNCHCTL, "bootout", domain_target, nimbuswm_plist_path, NULL};
        safe_exec((char *const *)args, false);
