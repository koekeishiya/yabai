...
static bool get_config_file(char *restrict filename, char *restrict buffer, int buffer_size)
{
    ...
    {
        snprintf(buffer, buffer_size, "%s/nimbuswm/%s", xdg_home, filename);
    ...
    snprintf(buffer, buffer_size, "%s/.config/nimbuswm/%s", home, filename);
    if (file_exists(buffer))
        return true;
...
{
    if (config_file[0] == '\0' && !get_config_file("nimbuswmrc", config_file, config_file_size))
    {
        warn("nimbuswm: could not locate config file..\n");
    ...
    {
        warn("nimbuswm: configuration file '%s' does not exist..\n", config_file);
    ...
    {
        warn("nimbuswm: failed to load config file '%s'\n", config_file);
...
#define AX_ENHANCED_UI_WORKAROUND(r, c)                                                       \
    {                                                                                         \
        ...
        if (eui)                                                                              \
            AXUIElementSetAttributeValue(r, kAXEnhancedUserInterface, kCFBooleanFalse);       \
        c if (eui) AXUIElementSetAttributeValue(r, kAXEnhancedUserInterface, kCFBooleanTrue); \
    }
