...
void window_manager_query_windows_for_displays(FILE *rsp, uint64_t flags)
{
    ...
    for (int i = 0; i < display_count; ++i)
    {
        ...
        //
        // NOTE(john-json): display_space_list(..) uses a linear allocator,
...
bool window_manager_rule_matches_window(struct rule *rule, struct window *window, char *window_title, char *window_role, char *window_subrole)
{
    ...
    if (regex_match(rule_check_flag(rule, RULE_APP_VALID), &rule->app_regex, window->application->name) == regex_match_app)
        return false;

    int regex_match_title = rule_check_flag(rule, RULE_TITLE_EXCLUDE) ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    if (regex_match(rule_check_flag(rule, RULE_TITLE_VALID), &rule->title_regex, window_title) == regex_match_title)
        return false;

    int regex_match_role = rule_check_flag(rule, RULE_ROLE_EXCLUDE) ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    if (regex_match(rule_check_flag(rule, RULE_ROLE_VALID), &rule->role_regex, window_role) == regex_match_role)
        return false;

    int regex_match_subrole = rule_check_flag(rule, RULE_SUBROLE_EXCLUDE) ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    if (regex_match(rule_check_flag(rule, RULE_SUBROLE_VALID), &rule->subrole_regex, window_subrole) == regex_match_subrole)
        return false;
    ...
    for (int i = 0; i < buf_len(wm->rules); ++i)
    {
        if (one_shot_rules || !rule_check_flag(&wm->rules[i], RULE_ONE_SHOT))
        {
            if (window_manager_rule_matches_window(&wm->rules[i], window, window_title, window_role, window_subrole))
            {
                ...
                {
                    if (!rule_check_flag(&wm->rules[i], RULE_ROLE_VALID) && !string_equals(window_role, "AXWindow"))
                        continue;
                    if (!rule_check_flag(&wm->rules[i], RULE_SUBROLE_VALID) && !string_equals(window_subrole, "AXStandardWindow"))
                        continue;
                }
...
void window_manager_resize_window_relative_internal(struct window *window, CGRect frame, int direction, float dx, float dy, bool animate)
{
    ...
    int x_mod = (direction & HANDLE_LEFT) ? -1 : (direction & HANDLE_RIGHT) ? 1
                                                                            : 0;
    int y_mod = (direction & HANDLE_TOP) ? -1 : (direction & HANDLE_BOTTOM) ? 1
                                                                            : 0;
...
{
    //
    // NOTE(john-json): Attempting to check the window frame cache to prevent unnecessary movement and resize calls to the AX API
    ...
    AX_ENHANCED_UI_WORKAROUND(window->application->ref, {
        // NOTE(john-json): Due to macOS constraints (visible screen-area), we might need to resize the window *before* moving it.
        window_manager_resize_window(window, width, height);

        window_manager_move_window(window, x, y);

        // NOTE(john-json): Due to macOS constraints (visible screen-area), we might need to resize the window *after* moving it.
        window_manager_resize_window(window, width, height);
    });
}
...
struct window *window_manager_find_first_nephew_for_managed_window(struct window_manager *wm, struct window *window)
{
    ...
    struct window_node *sibling_node = window_node_is_left_child(node) ? node->parent->right : node->parent->left;
    if (window_node_is_leaf(sibling_node) || !window_node_is_leaf(sibling_node->left))
        return NULL;
...
struct window *window_manager_find_second_nephew_for_managed_window(struct window_manager *wm, struct window *window)
{
    ...
    struct window_node *sibling_node = window_node_is_left_child(node) ? node->parent->right : node->parent->left;
    if (window_node_is_leaf(sibling_node) || !window_node_is_leaf(sibling_node->right))
        return NULL;
...
struct window *window_manager_find_second_cousin_for_managed_window(struct window_manager *wm, struct window *window)
{
    ...
    struct window_node *uncle_node = window_node_is_left_child(node->parent) ? grandparent->right : grandparent->left;
    if (window_node_is_leaf(uncle_node) || !window_node_is_leaf(uncle_node->right))
        return NULL;
...
static void window_manager_make_key_window(ProcessSerialNumber *window_psn, uint32_t window_id)
{
    ...
    // :SynthesizedEvent
    //
    // NOTE(john-json): These events will be picked up by an event-tap
...
struct window *window_manager_create_and_add_window(struct space_manager *sm, struct window_manager *wm, struct application *application, AXUIElementRef window_ref, uint32_t window_id, bool one_shot_rules)
{
    ...
    //
    // NOTE(john-json): Attempt to track **all** windows.
    //
    ...
    //
    // NOTE(john-json): However, only **root windows** are eligible for management.
    //
    ...
    if (window->is_root)
    {
        ...
        //
        // NOTE(john-json): A lot of windows misreport their accessibility role, so we allow the user
        ...
        else
        {
            ...
            //
            // NOTE(john-json): Print window information when debug_output is enabled.
...
bool window_manager_add_existing_application_windows(struct space_manager *sm, struct window_manager *wm, struct application *application, int refresh_index)
{
    ...
    for (int i = 0; i < window_count; ++i)
    {
        ...
        // :Workaround
        //
        // NOTE(john-json): The AX API appears to always include a single element for Finder that returns an empty window id.
    ...
    if (global_window_count != window_count - empty_count)
    {
        if (refresh_index == -1)
        {
            ...
            if (missing_window)
            {
                ...
                //
                // NOTE(john-json): MacOS API does not return AXUIElementRef of windows on inactive spaces.
...
enum window_op_error window_manager_warp_window(struct space_manager *sm, struct window_manager *wm, struct window *a, struct window *b)
{
    ...
    else
    {
        if (a_view->sid == b_view->sid)
        {
            ...
            // :NaturalWarp
            //
            // NOTE(john-json): Precalculate both target areas and select the one that has the closest distance to the source area.
        ...
        else
        {
            ...
            // :NaturalWarp
            //
            // TODO(john-json): Warp operations with operands that belong to different monitors does not yet implement a heuristic to select
...
void window_manager_wait_for_native_fullscreen_transition(struct window *window)
{
        ...
        workspace_is_macos_sequoia())
    {
        while (!space_is_user(space_manager_active_space()))
        {
            ...
            //
            // NOTE(john-json): Window has exited native-fullscreen mode.
...
void window_manager_toggle_window_native_fullscreen(struct window *window)
{
    ...
    //
    // NOTE(john-json): The window must become the focused window
    ...
    //
    // NOTE(john-json): We toggled the fullscreen attribute and must
...
bool window_manager_toggle_scratchpad_window(struct window_manager *wm, struct window *window, int forced_mode)
{
    ...
    // TODO(john-json): Both functions use the same underlying API and could be combined in a single function to reduce redundant work.
