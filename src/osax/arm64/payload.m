...
const char *get_dock_spaces_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 12) {
        return "55 21 00 ?? B5 02 33 91 A0 02 40 F9 ?? 1F 00 ?? 01 C1 47 F9 E2 03 1B AA ?? ?? 0C 94 FD 03 1D AA ?? ?? 0C 94 E0 13 00 F9 08 20 00 ?? 00 ?? 40 F9 ?? ?? 0C 94 E8 1F 00 ?? 13 F1 40 F9 E1 03 13 AA 02 00 80 D2 ?? ?? 0C 94 E0 27 00 F9 A0 02 40 F9 08 20 00 ?? 01 7D 42 F9";
    }
...
const char *get_dppm_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 12) {
        return "40 21 00 D0 00 00 35 91 E1 03 13 AA ?? ?? 0C 94 D3 30 00 B4 08 20 00 F0 00 ?? ?? F9 E8 1F 00 B0 19 05 47 F9 E1 03 19 AA ?? ?? 0C 94 FD 03 1D AA ?? ?? 0C 94 F4 03 00 AA FF 6F 01 B9 ?? ?? 0C 94 A1 43 02 D1 ?? ?? 0C 94 15 21 00 D0 00 01 00 35 C1 1A 00 F0 21 ?? 1F 91";
    }
...
const char *get_move_space_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 12) {
        return "7F 23 03 D5 E3 03 1E AA ?? ?? 00 94 FE 03 03 AA FD 7B 06 A9 FD 83 01 91 F6 03 14 AA F4 03 02 AA FA 03 01 AA FB 03 00 AA 97 0A 00 B0 F7 ?? ?? 91 E8 02 40 F9 18 68 68 F8 E0 03 18 AA E1 03 16 AA ?? 2C 00 94 80 01 00 B4 F3 03 00 AA F5 03 01 AA 08 0B 00 B0 08 A1 26 91 08 01 40 39 1F 05 00 71 E1 00 00 54 ?? 62 00 94 ?? 36 01 94 ?? 62 00 94 CF 00 00 14 14 00 80 52 D2 00 00 14 1A 01 00 B4 E8 02 40 F9 40 6B 68 F8 E1 03 16 AA ?? 2C 00 94";
    }
...
const char *get_set_front_window_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 12) {
        return "7F 23 03 D5 FF 43 02 D1 F6 57 06 A9 F4 4F 07 A9 FD 7B 08 A9 FD 03 02 91 A8 1A 00 90 08 ?? 44 F9 08 01 40 F9 A8 83 1D F8 C1 10 00 34 F3 03 01 AA F4 03 00 AA 15 FC 60 D3 FF BF 00 39 E1 BF 00 91 72 A1 00 94 A0 1A 00 90 00 ?? 45 F9 21 00 80 52 ?? ?? 0B 94 20 0A 00 36 F6 BF 40 39 E8 83 06 32 E8 53 06 29 08 80 80 52 E8 73 00 79 F5 A3 03 B8 E8 7F 00 79 F3 43 00 B9 E8 8B 00 79 F6 63 04 B8 ?? ?? D8 10";
    }
