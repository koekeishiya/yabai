    ...
    if (!__atomic_load_n(&process->ns_application, __ATOMIC_RELAXED))
    {
        ...
        if (!__atomic_load_n(&process->ns_application, __ATOMIC_RELAXED))
        {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1f * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
              ...
              if (_process)
                  event_loop_post(&g_event_loop, APPLICATION_LAUNCHED, _process, 0);
            });
...
static EVENT_HANDLER(WINDOW_MOVED)
{
    ...
    if (!windowed_fullscreen)
    {
        ...
        if (!g_mouse_state.window || g_mouse_state.window != window)
        {
            ...
            {
                struct window_node *node = view_find_window_node(view, window->id);
                if (node && (AX_DIFF(node->area.x, new_origin.x) || AX_DIFF(node->area.y, new_origin.y)) &&
    ...
    else if (!was_fullscreen == !is_fullscreen)
    {
        ...
        if (!windowed_fullscreen)
        {
            ...
            if (!g_mouse_state.window || g_mouse_state.window != window)
            {
                ...
                {
                    struct window_node *node = view_find_window_node(view, window->id);
                    if (node && (AX_DIFF(node->area.x, new_frame.origin.x) || AX_DIFF(node->area.y, new_frame.origin.y) || AX_DIFF(node->area.w, new_frame.size.width) || AX_DIFF(node->area.h, new_frame.size.height)) &&
...
static EVENT_HANDLER(MOUSE_DOWN)
{
    ...
    if (!window || window_check_flag(window, WINDOW_FULLSCREEN))
        goto out;
    ...
    if (g_mouse_state.current_action == MOUSE_MODE_RESIZE)
    {
        ...
        if (point.x < frame_mid.x)
            g_mouse_state.direction |= HANDLE_LEFT;
        if (point.y < frame_mid.y)
            g_mouse_state.direction |= HANDLE_TOP;
        if (point.x > frame_mid.x)
            g_mouse_state.direction |= HANDLE_RIGHT;
        if (point.y > frame_mid.y)
            g_mouse_state.direction |= HANDLE_BOTTOM;
    }
...
static EVENT_HANDLER(MOUSE_MOVED)
{
        ...
        else if (g_window_manager.ffm_mode == FFM_AUTORAISE)
        {
            ...
            if (window_list)
            {
                for (int i = 0; i < window_count; ++i)
                {
                    ...
                    if (!window_check_flag(sub_window, WINDOW_FLOAT))
                        continue;
                    if (window_level(window->id) != window_level(sub_window->id))
                        continue;
                    if (window_sub_level(window->id) != window_sub_level(sub_window->id))
                        continue;
    ...
    else
    {
        ...
        if (!wid)
            display_manager_set_active_display_id(cursor_did);
...
static void *event_loop_run(void *context)
{
    ...
    while (event_loop->is_running)
    {
        ...
        for (;;)
        {
            ...
            {
#define EVENT_TYPE_ENTRY(value)                                                                                                     \
    case value:                                                                                                                     \
        EVENT_HANDLER_##value(__atomic_load_n(&next->context, __ATOMIC_RELAXED), __atomic_load_n(&next->param1, __ATOMIC_RELAXED)); \
        break;
                EVENT_TYPE_LIST
...
{
    if (!memory_pool_init(&event_loop->pool, KILOBYTES(512)))
        return false;

    event_loop->semaphore = sem_open("nimbuswm_event_loop_semaphore", O_CREAT, 0600, 0);
    sem_unlink("nimbuswm_event_loop_semaphore");
    if (event_loop->semaphore == SEM_FAILED)
        return false;
