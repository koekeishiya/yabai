...
*nimbuswm* [*--load-sa*|*--uninstall-sa*|*--install-service*|*--uninstall-service*|*--start-service*|*--restart-service*|*--stop-service*|*--message*,*-m* 'msg'|*--config*,*-c* 'config_file'|*--verbose*,*-V*|*--version*,*-v*|*--help*,*-h*]

Description
-----------

*nimbuswm* is a tiling window manager for macOS based on binary space partitioning.
...
*--install-service*::
    ...
    Path is ~/Library/LaunchAgents/com.john-json.nimbuswm.plist.
...
nimbuswm -m config [--space '<SPACE_SEL>'] <space setting>::
...
nimbuswm -m display ['<DISPLAY_SEL'>] '<COMMAND>'
...
nimbuswm -m space ['<SPACE_SEL>'] '<COMMAND>'
...
nimbuswm -m window ['<WINDOW_SEL>'] '<COMMAND>'
...
nimbuswm -m query '<COMMAND>' ['<PROPERTIES>'] ['<ARGUMENT>']
...
until the window that space belongs to becomes active -- nimbuswm window commands will NOT WORK for these windows. +
...
If *nimbuswm* can't handle a message, it will return a non-zero exit code.
