...
*mouse_action* ['<integer number>']::
    List all registered mouse actions, or report the specified mouse action. Up to 16 mouse actions may be specified, using indices 0-15 inclusive.

*mouse_action* ['<integer number>'] 'clear'::
    Clear all registered mouse actions, or the specified mouse action.

*mouse_action* '<integer number>' ['alt|shift|ctrl|cmd|fn|super']+ ['left|right|middle'] ['move|move2|resize'] ['swap|stack']::
    Set the specified mouse action. Any combination of modifiers may be specified, delimited by spaces. 'super' is a shortcut for 'alt shift ctrl cmd'. A mouse button and drag action (move or resize) should also be specified, and a drop action (swap or stack) may optionally be specified. For example, *mouse_action 3 alt ctrl right resize*. Note that 'move2' is a legacy move action that can be used when the scripting addition is not installed, although it is less performant.
